        ...
        <form action="{{ route('customer.pesanan.store') }}" method="POST" enctype="multipart/form-data" id="checkoutForm">
            ...
            <div class="bg-white border border-slate-200 rounded-lg p-4 mb-6">
                <div class="flex items-center justify-between mb-3">
                    ...
                    <a href="{{ route('customer.alamat.index') }}" target="_blank"
                            ...
                            <div class="border border-slate-200 rounded-lg p-5 mb-6">
                                ...
                                @if ($layananPengiriman->isEmpty())
                                    <div class="text-center py-6">
                                        <p class="text-slate-500 text-sm">Tidak ada layanan pengiriman yang tersedia saat
                                            ini.</p>
                                    </div>
                                @else
                                    <div class="space-y-3">
                                        {{-- @foreach ($layananPengiriman as $layanan)
                                            <label
                                                ...
                                                <input type="radio" name="id_layanan_pengiriman"
                                                    value="{{ $layanan->id }}"
                                        ...
                                        @foreach ($layananPengiriman as $layanan)
                                            <label
                                                ...
                                                <input type="radio" name="id_layanan_pengiriman"
                                                    value="{{ $layanan->id }}"
                                                    class="h-4 w-4 text-teal-600 focus:ring-teal-500" {{-- PERUBAHAN DISINI: --}}
                                                    {{-- Gunakan $loop->first agar item pertama otomatis terpilih jika belum ada pilihan --}} @if ((isset($selectedLayananId) && $selectedLayananId == $layanan->id) || $loop->first) checked @endif
                                                    required>
                                                <div class="flex-1">
                                                    <p class="font-medium text-slate-700">{{ $layanan->nama_layanan }}</p>
                                                    <p class="text-xs text-slate-500">Tarif: Rp
                            ...
                            <div class="bg-slate-50 border border-slate-200 rounded-lg p-5 sticky top-24">
                                ...
                                <div class="space-y-3 text-sm">
                                    <div class="flex justify-between">
                                        <span class="text-slate-600">Total Harga ({{ count($selectedItems) }}
                                            barang)</span>
                                    ...
                                    <div class="bg-white rounded p-3 border border-slate-200">
                                        ...
                                        <div class="flex justify-between">
                                            ...
                                            <span class="font-medium" id="ongkirDisplay">Rp
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('input[name="id_layanan_pengiriman"]').forEach(radio => {
                radio.addEventListener('change', function() {
                    // document.getElementById('selectedLayananInput').value = this.value; // HAPUS INI JUGA
                    calculateAndUpdateOngkir(this.value);
                });
            });
                ...
                fetch('{{ route('customer.pesanan.calculateOngkir') }}', {
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            document.getElementById('tarifDisplay').textContent = 'Rp ' + tarif.toLocaleString(
                                'id-ID') + '/km';
                            document.getElementById('ongkirDisplay').textContent = data.data
                                .total_ongkir_formatted;
                            document.getElementById('totalDisplay').textContent = 'Rp ' + grandTotal

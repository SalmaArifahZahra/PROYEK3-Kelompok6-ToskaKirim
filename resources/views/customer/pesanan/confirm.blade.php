                            ...
                            <div class="space-y-3 text-sm">
                                ...
                                <div class="bg-white rounded p-3 border border-slate-200">
                                    <div class="flex justify-between mb-2">
                                        <span class="text-slate-600 text-xs">Jarak:</span>
                                        <span class="font-medium text-xs" id="distanceDisplay">-</span>
                                    </div>
                                    <div class="flex justify-between mb-2">
                                        <span class="text-slate-600 text-xs">Tarif/km:</span>
        ...
        function calculateAndUpdateOngkir(layananId) {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    if(elDistance) elDistance.textContent = data.data.jarak.toFixed(3) + ' km';
                    if(elTarif) elTarif.textContent = 'Rp ' + data.data.tarif_per_km.toLocaleString('id-ID') + '/km';
                    if(elOngkir) elOngkir.textContent = data.data.total_ongkir_formatted;
                    if(elTotal) elTotal.textContent = 'Rp ' + grandTotal.toLocaleString('id-ID');
                } else {
                    Swal.fire({ icon: 'error', title: 'Gagal Hitung Ongkir', text: data.error });
                }
            })
            .catch(error => console.error('Ongkir Error:', error));
        }

<aside class="fixed left-0 z-40 w-64 bg-white shadow-lg"
    ...
    @php
        $user = Auth::user();
        // Cek Role Superadmin
        $isSuperadmin = $user->peran === \App\Enums\RoleEnum::SUPERADMIN ?? 'superadmin';
    @endphp
    ...
    <nav class="flex flex-col h-full justify-between py-6 overflow-y-auto">
        ...
        <div class="px-4 space-y-2">
            ...
            @if($isSuperadmin)
                ...
                {{-- Header Operasional Toko (Pemisah sebelum masuk menu umum) --}}
                <div class="pt-4 pb-2">
                    <p class="px-4 text-xs font-semibold text-gray-400 uppercase tracking-wider">
                        Operasional Toko
                    </p>
        ...
        <div class="px-4 space-y-2 border-t border-gray-200 pt-4">
            ...
            {{-- Pengaturan (Disembunyikan untuk superadmin karena sudah ada Kontrol Toko di atas, atau biarkan jika ada fungsi lain) --}}
            @if(!$isSuperadmin)
                <a href="#" class="flex items-center gap-3 px-4 py-3 rounded-lg transition-colors
